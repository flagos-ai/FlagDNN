...
# (shape, threshold_val, value_val, inplace) 的组合测试用例
THRESHOLD_CASES = [
    # 1. 基础与经典形状
    ((1024,), 0.0, 0.0, False),  # 经典 ReLU 行为
    ((1024,), 1.5, 99.0, True),  # 一维 inplace 测试
    # 2. 非对齐的形状
    ((1023,), -1.0, 5.0, False),
    ((17, 31), 0.5, -2.5, True),
    ((13, 17, 19, 23), 0.0, 42.0, True),  # 4D 全质数形状，绝对的非 2 的幂次方
    # 3. 多维与高维张量
    ((4, 8, 16), 2.0, 0.0, False),
    ((2, 3, 4, 5), -0.5, 10.0, True),
    (
        (1, 1, 1, 1, 1),
        0.5,
        -0.5,
    ...
    ((1024 * 1024,), 1.0, -1.0, False),  # 百万级一维元素，测试 Grid 拆分机制
    (
        (64, 128, 256),
        0.5,
        0.0,
        True,
    ),  # 大体积 3D 张量，测试 inplace 下的大吞吐量
    # 5. 极端条件判断 (All or Nothing)
    ((64, 64), 100.0, -1.0, False),  # 阈值极大 (100.0)：几乎所有元素都会被替换
    (
        (64, 64),
        -100.0,
        -1.0,
    ...
    ((256, 256), 0.0, float("inf"), True),  # 替换为正无穷
    # 7. 极端边界情况
    ((1,), 0.0, 1.0, False),  # 单元素
    (
        (0,),
...
@pytest.mark.parametrize(
    "shape, threshold_val, value_val, inplace", THRESHOLD_CASES
)
def test_accuracy_threshold(dtype, shape, threshold_val, value_val, inplace):
    ...
    out_ref = F.threshold(x_ref, threshold_val, value_val, inplace=inplace)

    with flag_dnn.use_dnn():
        out_custom = F.threshold(
            x_custom, threshold_val, value_val, inplace=inplace
        )
