...
def _run_flag_dnn_max_graph(x, y):
    @flag_dnn.graph
    def flag_dnn_max_graph(x, y):
        return flag_dnn.max(
            input0=x,
            input1=y,
    ...
    assert [node.op_type for node in compiled.graph.nodes] == ["max"]
    assert compiled.graph.nodes[0].attrs["compute_data_type"] == "float32"
    assert compiled.graph.nodes[0].attrs["name"] == "max"
...
def test_max(cudnn_handle, dtype, case):
    ...
    cudnn_out = _cudnn_max(x, y, cudnn_handle)
    flag_dnn_out = _run_flag_dnn_max_graph(x, y)
