...
def _run_flag_dnn_add_square_graph(x, y):
    @flag_dnn.graph
    def flag_dnn_add_square_graph(x, y):
        return flag_dnn.add_square(
            a=x,
            b=y,
    ...
    assert compiled.graph.nodes[0].attrs["name"] == "add_square"
...
def test_add_square(cudnn_handle, dtype, case):
    ...
    cudnn_out = _cudnn_add_square(x, y, cudnn_handle)
    flag_dnn_out = _run_flag_dnn_add_square_graph(x, y)
