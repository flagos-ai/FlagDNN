...
DNN_COMPARE_DTYPES = (torch.float16, torch.bfloat16, torch.float32)
...
ADD_COMMON_CASES = POINTWISE_BINARY_CASES + BROADCAST_BINARY_CASES
ADD_EXTRA_CASES_BY_VENDOR = {
    "ascend": (
        ((2, 3, 17), (1, 1, 17)),
        ((2, 3, 5, 7), (1, 3, 1, 7)),
    ),
...
def get_add_test_cases(vendor_name=None):
    if vendor_name is None:
        import flag_dnn

        vendor_name = flag_dnn.vendor_name
    return ADD_COMMON_CASES + ADD_EXTRA_CASES_BY_VENDOR.get(vendor_name, ())
