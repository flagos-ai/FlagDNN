...
def test_accuracy_minimum(shape, dtype):
    ...
    ref_out = torch.minimum(ref_x, ref_y)
    with flag_dnn.use_dnn():
        res_out = torch.minimum(x, y)
...
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_minimum_int(shape, dtype):
    ...
    with flag_dnn.use_dnn():
        res_out = torch.minimum(x, y)
    utils.gems_assert_equal(res_out, ref_out.to(dtype))


@pytest.mark.minimum
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("input_shape, other_shape", BROADCAST_SHAPES)
def test_accuracy_minimum_broadcast(input_shape, other_shape, dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
...
@pytest.mark.minimum
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_minimum_empty_tensor(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
