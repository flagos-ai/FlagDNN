...
def test_accuracy_maximum(shape, dtype):
    ...
    ref_out = torch.maximum(ref_x, ref_y)
    with flag_dnn.use_dnn():
        res_out = torch.maximum(x, y)
...
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_maximum_int(shape, dtype):
...
def test_accuracy_maximum_broadcast(input_shape, other_shape, dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
...
@pytest.mark.maximum
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_maximum_empty_tensor(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
