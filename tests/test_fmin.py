...
def test_accuracy_fmin(shape, dtype):
    ...
    ref_out = torch.fmin(ref_x, ref_y)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_fmin_with_nan(shape, dtype):
    """Test NaN-aware behavior: fmin ignores NaN."""
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
    x = torch.randn(shape, dtype=dtype, device=flag_dnn.device)
    y = torch.randn(shape, dtype=dtype, device=flag_dnn.device)
...
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_fmin_int(shape, dtype):
...
def test_accuracy_fmin_broadcast(input_shape, other_shape, dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
...
@pytest.mark.fmin
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_fmin_empty_tensor(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")
