...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_isinf_with_posinf(shape, dtype):
    """包含正无穷值"""
    _skip_fp64(dtype)
    x = torch.randn(shape, dtype=dtype, device=flag_dnn.device)
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_isinf_with_neginf(shape, dtype):
    """包含负无穷值"""
    _skip_fp64(dtype)
    x = torch.randn(shape, dtype=dtype, device=flag_dnn.device)
    x.view(-1)[::10] = float("-inf")
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_isinf_with_nan(shape, dtype):
    """包含 NaN：isinf(nan) 应为 False"""
    _skip_fp64(dtype)
    x = torch.randn(shape, dtype=dtype, device=flag_dnn.device)
...
@pytest.mark.isinf
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_isinf_empty_tensor(dtype):
    """边界：空张量"""
    _skip_fp64(dtype)
