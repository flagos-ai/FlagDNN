...
def test_rmsnorm_rht_amax_wrapper_sm100_matches_reference(rows_per_cta):
    torch.manual_seed(8)
    m = 8
    n = 2048
    eps = 1e-5
    ...
    actual = flag_dnn.rmsnorm_rht_amax_wrapper_sm100(
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required")
def test_rmsnorm_rht_amax_wrapper_sm100_squeezes_trailing_unit_dims():
    torch.manual_seed(9)
    m = 4
    n = 2048
    eps = 1e-5
