...
def test_accuracy_bitwise_and(shape, dtype):
    ...
    ref_out = torch.bitwise_and(ref_x, ref_y)
    with flag_dnn.use_dnn():
        res_out = torch.bitwise_and(x, y)
    utils.gems_assert_equal(res_out, ref_out)


@pytest.mark.bitwise_and
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_bitwise_and_bool(shape):
...
@pytest.mark.parametrize("dtype", INT_DTYPES)
@pytest.mark.parametrize("shape", SHAPES)
def test_accuracy_bitwise_and_zeros(shape, dtype):
...
def test_accuracy_bitwise_and_broadcast(input_shape, other_shape, dtype):
    info = torch.iinfo(dtype)
    low = max(-100, info.min)
    high = min(100, info.max)
...
@pytest.mark.bitwise_and
@pytest.mark.parametrize("dtype", INT_DTYPES)
def test_accuracy_bitwise_and_empty_tensor(dtype):
