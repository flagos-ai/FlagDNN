...
def test_graph_sdpa_default_scale_with_stats(cudnn_handle, dtype, shape):
    torch.manual_seed(0)
...
@pytest.mark.parametrize("dtype", CUDNN_COMPARE_DTYPES)
@pytest.mark.parametrize("shape", consts.SDPA_CASES)
def test_graph_sdpa_inference(cudnn_handle, dtype, shape):
    torch.manual_seed(1)
    q, k, v = _make_qkv(shape, dtype)
...
@pytest.mark.parametrize("dtype", CUDNN_COMPARE_DTYPES)
@pytest.mark.parametrize("shape", consts.SDPA_MASKED_CASES)
def test_graph_sdpa_causal_bottom_right(cudnn_handle, dtype, shape):
    torch.manual_seed(3)
    batch, heads_q, heads_kv, seq_q, seq_kv, head_dim = shape
    if seq_q > seq_kv:
...
@pytest.mark.parametrize("dtype", CUDNN_COMPARE_DTYPES)
@pytest.mark.parametrize("shape", consts.SDPA_MASKED_CASES)
def test_graph_sdpa_sliding_window(cudnn_handle, dtype, shape):
    torch.manual_seed(4)
    q, k, v = _make_qkv(shape, dtype)
    cudnn_out = _cudnn_sdpa(
...
@pytest.mark.parametrize("dtype", CUDNN_COMPARE_DTYPES)
@pytest.mark.parametrize("shape", consts.SDPA_MASKED_CASES)
def test_graph_sdpa_bias(cudnn_handle, dtype, shape):
    if dtype == torch.float32:
        ...
        # torch reference in test_graph_sdpa_bias_float32 instead.
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required")
@pytest.mark.parametrize("shape", consts.SDPA_MASKED_CASES)
def test_graph_sdpa_bias_float32(shape):
    dtype = torch.float32
    torch.manual_seed(5)
    batch, heads_q, heads_kv, seq_q, seq_kv, head_dim = shape
    q, k, v = _make_qkv(shape, dtype)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required")
@pytest.mark.parametrize("dtype", CUDNN_COMPARE_DTYPES)
def test_graph_sdpa_explicit_scale(cudnn_handle, dtype):
    torch.manual_seed(6)
...
def test_graph_sdpa_causal_shorthand_matches_band_form(cudnn_handle, dtype):
    del cudnn_handle
    torch.manual_seed(7)
