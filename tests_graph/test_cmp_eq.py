...
def _run_flag_dnn_cmp_eq_graph(x, y):
    @flag_dnn.graph
    def flag_dnn_cmp_eq_graph(x, y):
        return flag_dnn.cmp_eq(
            input=x,
            comparison=y,
    ...
    assert [node.op_type for node in compiled.graph.nodes] == ["cmp_eq"]
    assert compiled.graph.nodes[0].attrs["compute_data_type"] == "float32"
    assert compiled.graph.nodes[0].attrs["name"] == "cmp_eq"
...
def test_graph_cmp_eq_matches_cudnn_frontend(cudnn_handle, dtype, case):
    torch.manual_seed(0)
    x, y = _make_inputs(case, dtype)

    cudnn_out = _cudnn_cmp_eq(x, y, cudnn_handle)
    flag_dnn_out = _run_flag_dnn_cmp_eq_graph(x, y)
