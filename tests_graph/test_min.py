...
def _run_flag_dnn_min_graph(x, y):
    @flag_dnn.graph
    def flag_dnn_min_graph(x, y):
        return flag_dnn.min(
            input0=x,
            input1=y,
    ...
    assert [node.op_type for node in compiled.graph.nodes] == ["min"]
    assert compiled.graph.nodes[0].attrs["compute_data_type"] == "float32"
    assert compiled.graph.nodes[0].attrs["name"] == "min"
...
@pytest.mark.parametrize("case", consts.MIN_CASES)
def test_graph_min_matches_cudnn_frontend(cudnn_handle, dtype, case):
    torch.manual_seed(0)
    ...
    cudnn_out = _cudnn_min(x, y, cudnn_handle)
    flag_dnn_out = _run_flag_dnn_min_graph(x, y)
