...
def test_graph_sdpa_backward(cudnn_handle, dtype, shape):
    torch.manual_seed(0)
...
def test_graph_sdpa_backward_causal(cudnn_handle, dtype, shape):
    torch.manual_seed(1)
    q, k, v = _make_qkv(shape, dtype)
    dO = torch.randn(
        (shape[0], shape[1], shape[3], shape[5]),
...
def test_graph_sdpa_backward_bias_dbias(cudnn_handle, dtype):
    torch.manual_seed(2)
