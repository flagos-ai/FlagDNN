...
def _cudnn_cmp_ge(x, y, cudnn_handle):
...
def _run_flag_dnn_cmp_ge_graph(x, y):
    @flag_dnn.graph
    ...
    assert compiled.graph.nodes[0].attrs["name"] == "cmp_ge"
...
def test_graph_cmp_ge_matches_cudnn_frontend(cudnn_handle, dtype, case):
    torch.manual_seed(0)
    x, y = _make_inputs(case, dtype)

    cudnn_out = _cudnn_cmp_ge(x, y, cudnn_handle)
    flag_dnn_out = _run_flag_dnn_cmp_ge_graph(x, y)
