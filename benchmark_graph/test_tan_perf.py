...
class TanBenchmark(CudnnCompareBenchmark):
    op_name = "tan"
    shapes = consts.TAN_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_TAN_PERF_SHAPE_IDS"
...
def test_perf_graph_tan_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    TanBenchmark(cudnn_handle).run(dtype)
