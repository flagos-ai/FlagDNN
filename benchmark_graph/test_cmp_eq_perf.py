...
class CmpEqBenchmark(CudnnCompareBenchmark):
    op_name = "cmp_eq"
    shapes = consts.CMP_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_CMP_EQ_PERF_SHAPE_IDS"
...
def test_perf_graph_cmp_eq_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    CmpEqBenchmark(cudnn_handle).run(dtype)
