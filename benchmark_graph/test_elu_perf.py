...
class EluBenchmark(CudnnCompareBenchmark):
    op_name = "elu"
    shapes = consts.ELU_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_ELU_PERF_SHAPE_IDS"
...
def test_perf_graph_elu_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    EluBenchmark(cudnn_handle).run(dtype)
