...
class SwishBenchmark(CudnnCompareBenchmark):
    op_name = "swish"
    shapes = consts.SWISH_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_SWISH_PERF_SHAPE_IDS"
...
def test_perf_graph_swish_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    SwishBenchmark(cudnn_handle).run(dtype)
