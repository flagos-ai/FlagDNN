...
class LogicalNotBenchmark(CudnnCompareBenchmark):
    op_name = "logical_not"
    dtypes = (torch.bool,)
    shapes = consts.EXP_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_LOGICAL_NOT_PERF_SHAPE_IDS"
...
def test_perf_graph_logical_not_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    LogicalNotBenchmark(cudnn_handle).run(dtype)
