...
class RsqrtBenchmark(CudnnCompareBenchmark):
    op_name = "rsqrt"
    shapes = consts.RSQRT_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_RSQRT_PERF_SHAPE_IDS"
...
def test_perf_graph_rsqrt_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    RsqrtBenchmark(cudnn_handle).run(dtype)
