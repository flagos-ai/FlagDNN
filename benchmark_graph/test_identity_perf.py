...
class IdentityBenchmark(CudnnCompareBenchmark):
    op_name = "identity"
    shapes = consts.CUDNN_IDENTITY_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_IDENTITY_PERF_SHAPE_IDS"
...
def test_perf_graph_identity_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    IdentityBenchmark(cudnn_handle).run(dtype)
