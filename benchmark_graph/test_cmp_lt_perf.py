...
class CmpLtBenchmark(CudnnCompareBenchmark):
    op_name = "cmp_lt"
    shapes = consts.CUDNN_CMP_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_CMP_LT_PERF_SHAPE_IDS"
...
def test_perf_graph_cmp_lt_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    CmpLtBenchmark(cudnn_handle).run(dtype)
