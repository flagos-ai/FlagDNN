...
class MulBenchmark(CudnnCompareBenchmark):
    op_name = "mul"
    shapes = consts.CUDNN_MUL_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_MUL_PERF_SHAPE_IDS"
...
def test_perf_graph_mul_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    MulBenchmark(cudnn_handle).run(dtype)
