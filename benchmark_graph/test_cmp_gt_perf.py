...
class CmpGtBenchmark(CudnnCompareBenchmark):
    op_name = "cmp_gt"
    shapes = consts.CUDNN_CMP_SHAPES
    shape_ids_env = "FLAGDNN_CUDNN_CMP_GT_PERF_SHAPE_IDS"
...
@pytest.mark.parametrize("dtype", CmpGtBenchmark.dtypes)
def test_perf_graph_cmp_gt_vs_cudnn_frontend(cudnn_handle, dtype):
    torch.manual_seed(0)
    CmpGtBenchmark(cudnn_handle).run(dtype)
