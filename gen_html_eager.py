...
# ============ 配置区 ============
SUMMARY_FILE = "perf_summary.json"
...
def normalize_op_name_from_file(filename: str) -> str:
    """
    从 perf_summary.json 的 file 字段中提取算子名。
...
def normalize_op_name(op: str) -> str:
    """
    兼容 perf_data.json 中 operator 字段的不同写法。
...
def get_first_existing(record, keys, default=None):
    ...
    主要用于兼容 perf_data.json 里的 dtype 字段名不固定的情况。
    """
    for key in keys:
...
def main():
    ...
    # 2. 解析 perf_data.json
    # 只统计 perf_summary.json 里 PASS 的算子。
    # 如果 perf_summary.json 里 details 为空，则退化为统计 perf_data.json 中所有算子。
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .section-title { font-size: 1.1rem; color: #4a5568; margin-bottom: 20px; padding-bottom: 10px;  .two-col { grid-template-columns: 1fr; }
...
"""
    ...
    print(f"🎉 成功！合并报告已生成至: {OUTPUT_HTML}")
