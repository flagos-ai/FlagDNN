...
def prod(
    ...
    *,
    ...
    def _launch_kernel_split_n(M, N, input_view, is_3d=False, I_dim=1):
        ...
        partial_buffer = torch.empty((M, num_tiles), dtype=acc_dtype, device=input_view.device)

        if not is_3d:
            grid = (triton.cdiv(M, BLOCK_M_SPLIT) * num_tiles, )
            _prod_kernel_2d_split_stage1[grid](
                input_view, partial_buffer,
                M, N, num_tiles,
                input_view.stride(0), input_view.stride(1),
            ...
            _prod_kernel_3d_split_stage1[grid](
                input_view, partial_buffer,
                M, N, I_dim, num_tiles,
                input_view.stride(0), input_view.stride(1), input_view.stride(2),
                BLOCK_M=BLOCK_M_SPLIT,
                BLOCK_N=BLOCK_N_SPLIT,
                num_warps=8,
            )
        ...
        _prod_kernel_2d_loop[grid_2d_loop](
            partial_buffer, out_buffer,
            M, num_tiles,
            partial_buffer.stride(0), partial_buffer.stride(1)
        )
        ...
        if not is_3d:
            ...
            if use_inner_parallel:
                ...
                _prod_kernel_3d_loop[grid_3d_loop](
                    input_view, out_buffer, M, N, I_dim,
                    input_view.stride(0), input_view.stride(1), input_view.stride(2)
                )
    ...
    with torch_device_fn.device(input.device):
        if is_reduce_at_end:
            M, N = 1, 1
            for i in range(ndim - len(dims)): M *= input.shape[i]
            for i in range(ndim - len(dims), ndim): N *= input.shape[i]
            out_buffer = _launch_kernel(M, N, input.reshape(M, N), is_3d=False)
        else:
            dim_min, dim_max = dims[0], dims[-1]
            O, R, I_dim = 1, 1, 1
            for j in range(dim_min): O *= input.shape[j]
            for j in range(dim_min, dim_max + 1): R *= input.shape[j]
            for j in range(dim_max + 1, ndim): I_dim *= input.shape[j]
            M, N = O * I_dim, R
            out_buffer = _launch_kernel(M, N, input.reshape(O, R, I_dim), is_3d=True, I_dim=I_dim, O_dim=O)
