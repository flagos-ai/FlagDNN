...
def fmax(
    ...
    *,
    ...
    logger.debug("FLAG_DNN FMAX")

    if input.dtype.is_complex:
        raise RuntimeError(f"fmax not implemented for '{input.dtype}'")
    ...
    if input.shape == other.shape and input.shape == out_shape:
        ...
        with torch_device_fn.device(input.device):
            if input.dtype.is_floating_point:
                fmax_tensor_kernel[grid](input, other, out, n_elements)
            else:
                fmax_int_tensor_kernel[grid](input, other, out, n_elements)
        return out
    ...
    if n_collapsed_dims <= 2:
        ...
        return binary(input, other, op_type="fmax", out=out)
