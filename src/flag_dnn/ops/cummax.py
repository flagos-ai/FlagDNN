...
from flag_dnn import runtime  # noqa: F401
...
@triton.jit
def _cummax_combine(lv, li, rv, ri):
    """Associative combine for cumulative maximum."""
    r_nan = rv != rv
    l_nan = lv != lv
    # Match PyTorch CUDA cummax: tie -> take right (LATER index).
    # NaN propagation: right-NaN wins, left-NaN only wins if right is not NaN.
...
@libentry()
@triton.jit
def cummax_inner_single_kernel(
    x_ptr,
    v_ptr,
    i_ptr,
    M,
    ...
    v, i_local = tl.associative_scan(
        (x, idx_local), axis=0, combine_fn=_cummax_combine
    )
...
@libentry()
@triton.jit
def cummax_inner_carry_scan_kernel(
    carry_v_ptr,
    carry_i_ptr,
    M,
    NUM_BLOCKS,
    ...
    v, i = tl.associative_scan((v, i), axis=0, combine_fn=_cummax_combine)
...
def cummax_outer_1d_kernel(
    ...
    for n in range(N):
        ...
        # >= to pick the later index on ties
        take_right = tl.where(
...
def _identity_for(dtype: torch.dtype):
    if dtype.is_floating_point:
        return float("-inf")
    return _INT_IDENTITY_MIN[dtype]
...
def cummax(
    ...
    *,
    ...
    logger.debug(f"FLAG_DNN CUMMAX (dim={dim})")
    ...
    if input.ndim == 0:
        ...
        if out is not None:
            ...
            out[0].copy_(values)
            out[1].copy_(indices)
            return torch.return_types.cummax((out[0], out[1]))
        return torch.return_types.cummax((values, indices))
...
def _finalize_out(out, values, indices):
    ...
    return torch.return_types.cummax((uv, ui))
...
def _run_inner(
    ...
    cummax_inner_pass1_kernel[grid](
        x,
        v,
        i,
        carry_v,
    ...
    if num_blocks > 2:
        ...
        cummax_inner_carry_scan_kernel[(M,)](
            carry_v,
            carry_i,
            M,
            num_blocks,
    ...
    cummax_inner_pass2_kernel[grid](
        v,
        i,
        carry_v,
        carry_i,
