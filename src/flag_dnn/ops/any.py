...
def _any_direct_kernel(
    ...
    n_mask = n_offsets < N
    x = (tl.load(row_ptr + n_offsets * stride_xn, mask=n_mask, other=0) != 0).to(tl.int32)
...
def _any_direct_multi_kernel(
    ...
    x = tl.load(ptrs, mask=m_mask[:, None] & n_mask[None, :], other=0)
    nonzero = (x != 0).to(tl.int32)               # [BLOCK_M, BLOCK_N]
    result = tl.max(nonzero, axis=1)               # [BLOCK_M] per-row max
    tl.store(out_ptr + m_offsets, result.to(tl.int8), mask=m_mask)

@triton.jit
def _any_stage1_kernel(
    x_ptr,
    partial_ptr,
    M,
    N,
    ...
    x = (tl.load(row_ptr + n_offsets * stride_xn, mask=n_mask, other=0) != 0).to(tl.int32)
    local_any = tl.max(x, axis=0)

    num_blocks_n = tl.num_programs(1)
    tl.store(partial_ptr + pid_m * num_blocks_n + pid_n, local_any)


@triton.jit
def _any_finalize_kernel(
    partial_ptr,
    out_ptr,
    M,
    K,
    ...
    k_mask = k_offsets < K
    part = tl.load(partial_ptr + pid_m * K + k_offsets, mask=k_mask, other=0)
    result = tl.max(part, axis=0)
    tl.store(out_ptr + pid_m, result)


# ---- 2-stage column reduction for axis=0 cases ----

@triton.jit
def _any_col2d_s1_kernel(
    x_ptr, partial_ptr,
    M_red, M_out,
    BLOCK_M: tl.constexpr,
    BLOCK_N: tl.constexpr,
):
    """Stage 1: each CTA processes a [BLOCK_M, BLOCK_N] tile of [M_red, M_out].
    stride_row = M_out (contiguous), stride_col = 1 → coalesced reads.
    ...
    # Coalesced: consecutive cols stride=1, row stride=M_out
    ptrs = x_ptr + row_offsets[:, None] * M_out + col_offsets[None, :]
    mask_2d = row_mask[:, None] & col_mask[None, :]
    x = tl.load(ptrs, mask=mask_2d, other=0).to(tl.int8)
    tile_any = tl.max(x, axis=0).to(tl.int8)  # [BLOCK_N]

    # partial[pid_m, col] — row-major
    partial_ptrs = partial_ptr + pid_m * M_out + col_offsets
    tl.store(partial_ptrs, tile_any, mask=col_mask)


@triton.jit
def _any_col2d_s2_kernel(
    partial_ptr, out_ptr,
    n_groups, M_out,
    BLOCK_K: tl.constexpr,
    BLOCK_N: tl.constexpr,
):
    """Stage 2: reduce n_groups partial rows → output [M_out].
    Grid: (ceil(M_out/BLOCK_N),)"""
    pid_n = tle.program_id(0)
    col_offsets = pid_n * BLOCK_N + tl.arange(0, BLOCK_N)
    col_mask = col_offsets < M_out
    ...
    for k_start in range(0, n_groups, BLOCK_K):
        ...
        partial = tl.load(ptrs, mask=mask_2d, other=0).to(tl.int8)
        row_any = tl.max(partial, axis=0).to(tl.int8)
        acc = tl.maximum(acc, row_any)

    tl.store(out_ptr + col_offsets, acc, mask=col_mask)


def _any_reduce_2d(x: torch.Tensor, M: int, N: int) -> torch.Tensor:
    if N == 0:
        return _filled_bool((M,), False, x.device)
    ...
    if num_blocks_n == 1:
        if M <= 4096:
            # Multi-row: 8 rows/CTA → ~128 CTAs for M=1024 (vs 1024 single-row)
            BLOCK_M_MR = 8
            num_warps_mr = 8
            grid_mr = (triton.cdiv(M, BLOCK_M_MR),)
            with torch_device_fn.device(x.device):
                _any_direct_multi_kernel[grid_mr](
        ...
        grid2 = (M,)
        with torch_device_fn.device(x.device):
            _any_finalize_kernel[grid2](partial, out, M, num_blocks_n, BLOCK_K=BLOCK_K)

    return out.view(torch.bool)


@triton.jit
def _any_col_atomic_kernel(
    x_ptr, out_ptr,
    M_red, M_out,
    BLOCK_M: tl.constexpr,
    BLOCK_N: tl.constexpr,
):
    """Single-kernel column reduction using atomic_max.
    Grid: (ceil(M_out/BLOCK_N), ceil(M_red/BLOCK_M)) → many CTAs, 1 launch.
    Reads coalesced (stride_col=1). Atomic writes to int32 output."""
    pid_n = tle.program_id(0)
    pid_m = tle.program_id(1)
    ...
    ptrs = x_ptr + row_offsets[:, None] * M_out + col_offsets[None, :]
    x = (tl.load(ptrs, mask=row_mask[:, None] & col_mask[None, :], other=0) != 0).to(tl.int32)
    tile_any = tl.max(x, axis=0)  # [BLOCK_N]

    # Atomic max: any True → output becomes 1
    tl.atomic_max(out_ptr + col_offsets, tile_any, mask=col_mask)


def _any_col2d_reduce(x_flat: torch.Tensor, M_red: int, M_out: int) -> torch.Tensor:
    """Single-kernel atomic column reduction. High-occupancy (2048 CTAs for 1K×1K).
    Avoids 2-stage overhead. Output[j] = any(x_flat[:, j])."""
    if M_out == 0 or M_red == 0:
        return _filled_bool((M_out,), False, x_flat.device)

    BLOCK_M = 16
    BLOCK_N = 32
    grid = (triton.cdiv(M_out, BLOCK_N), triton.cdiv(M_red, BLOCK_M))

    # Initialize with 0 (no True yet)
    out = _filled_i32((M_out,), 0, x_flat.device)
    with torch_device_fn.device(x_flat.device):
        _any_col_atomic_kernel[grid](
...
def any(
    ...
    *,
    ...
    logger.debug(f"FLAG_DNN ANY (dim={dim})")
    ...
    if dim is None:
        ...
        result = _any_reduce_2d(x_flat.unsqueeze(0), 1, N)
        result = result.view(())
        ...
        if d == 1:
            result = _any_reduce_2d(input, M_rows, N_cols)
            if keepdim:
                result = result.unsqueeze(1)
        else:  # d == 0
            result = _any_col2d_reduce(input, M_rows, N_cols)
            if keepdim:
        ...
        # Check if reduction dims are all leading dims [0, 1, ..., k-1]
        dims_are_leading = (dims == list(range(len(dims))))

        if perm_view.is_contiguous():
            input_view = perm_view.view(M_out, N_red)
            result_flat = _any_reduce_2d(input_view, M_out, N_red)
        elif input.is_contiguous() and dims_are_leading:
            # Leading-dim reduction on contiguous input: use 2-stage column reduction
            # View as [N_red, M_out] for coalesced reads (stride_col=1)
            result_flat = _any_col2d_reduce(input.view(N_red, M_out), N_red, M_out)
        elif (
            num_blocks_n == 1
            and perm_view.ndim == 2
            and perm_view.stride(-1) == 1
        ):
            result_flat = _any_reduce_2d(perm_view, M_out, N_red)
        else:
