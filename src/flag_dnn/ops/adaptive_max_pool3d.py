...
def adaptive_max_pool3d_kernel(
    ...
    N, C, D, H, W,
    OD, OH, OW,
    OW_OH, OW_OH_OD, C_OW_OH_OD,  # 传入外部预计算好的常量，避免 GPU 内部乘法计算
    ...
    for kd in range(MAX_K_D):
        ...
        for kh in range(MAX_K_H):
            ...
            for kw in range(MAX_K_W):
                ...
                val = tl.load(x_ptr + load_idx, mask=valid_mask & in_window, other=-float('inf'))
...
def adaptive_max_pool3d(
    ...
    idx_ptr = input 
    if return_indices:
        indices = torch.empty((N, C, OD, OH, OW), dtype=torch.int64, device=input.device)
        idx_ptr = indices
