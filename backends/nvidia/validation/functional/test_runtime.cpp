...
void execute_once(const flagdnn::Executable& executable,
...
std::size_t storage_element_count(std::span<const std::int64_t> dimensions,
...
std::size_t logical_offset(std::size_t logical_index,
  ...
  for (std::size_t axis = dimensions.size(); axis != 0; --axis) {
    ...
    const std::size_t dimension = static_cast<std::size_t>(dimensions[current]);
...
flagdnn::TensorDescriptor make_tensor(
    std::int64_t uid, std::initializer_list<std::int64_t> dimensions,
...
flagdnn::Executable build_add_with_alpha(
    flagdnn::Handle const& handle, flagdnn::TensorDescriptor const& left,
    flagdnn::TensorDescriptor const& right,
    flagdnn::TensorDescriptor const& output, double alpha) {
  flagdnn::Graph graph;
...
flagdnn::Executable build_reduction_sum(
    flagdnn::Handle const& handle, flagdnn::TensorDescriptor const& input,
    std::int32_t axis, bool keep_dimensions,
    flagdnn::TensorDescriptor const& output) {
  flagdnn::Graph graph;
...
flagdnn::Executable build_reduction(flagdnn::Handle const& handle,
                                    flagdnn::TensorDescriptor const& input,
...
flagdnn::Executable build_conv2d_fprop(
    ...
    std::span<const std::int64_t, 2> dilation, std::int64_t groups,
    flagdnn::TensorDescriptor const& output) {
  flagdnn::Graph graph;
  graph.conv2d_fprop(input, filter, padding, stride, dilation, groups, output);
  graph.finalize();
  return flagdnn::Executable(handle, graph);
}

void require_close(const char* name, std::span<const float> actual,
  ...
  for (std::size_t index = 0; index < actual.size(); ++index) {
    ...
    if (!std::isfinite(error))
      throw std::runtime_error(std::string(name) +
                               " non-finite cuDNN comparison");
    maximum_error = std::max(maximum_error, error);
  }
  if (!std::isfinite(maximum_error) || maximum_error > tolerance) {
    std::ostringstream message;
    message << name << " verification failed: max_abs_error=" << maximum_error
            << ", tolerance=" << tolerance;
...
reference::TestTensor reference_tensor(
    std::int64_t uid, std::vector<std::int64_t> dimensions,
    flagdnnDataType_t type = FLAGDNN_DATA_FLOAT32,
    std::vector<std::int64_t> strides = {}) {
  if (strides.empty()) strides = contiguous_strides(dimensions);
  return {uid, type, dimensions, strides};
}
std::unique_ptr<reference::TestExecutable> pointwise_reference(
    ...
    reference::TestTensor output, double alpha = 1.0) {
  reference::PointwiseTestCase specification;
  specification.name = "runtime_contract";
  ...
  specification.alpha = alpha;
  return reference::build_pointwise_reference(specification);
}
std::unique_ptr<reference::TestExecutable> reduction_reference(
    reference::TestTensor input, reference::TestTensor output, int axis,
    bool keep_dimensions, flagdnnReductionMode_t mode = FLAGDNN_REDUCTION_ADD) {
  reference::ReductionTestCase specification;
  specification.name = "runtime_contract";
  specification.input = std::move(input);
  specification.output = std::move(output);
  specification.axis = axis;
  specification.keep_dimensions = keep_dimensions;
  specification.mode = mode;
  return reference::build_reduction_reference(specification);
}

void test_relu(const flagdnn::Executable& executable, CUstream stream) {
  ...
  auto cudnn = pointwise_reference(FLAGDNN_POINTWISE_RELU_FWD,
                                   {reference_tensor(1, {elements})},
                                   reference_tensor(2, {elements}));
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 2, elements, stream);
...
void test_relu_16bit(const flagdnn::Executable& executable, CUstream stream,
                     flagdnnDataType_t data_type, std::int64_t input_uid,
                     std::int64_t output_uid, const char* name) {
  if (data_type != FLAGDNN_DATA_FLOAT16 && data_type != FLAGDNN_DATA_BFLOAT16) {
  ...
  const std::array<std::uint16_t, 6> patterns =
      data_type == FLAGDNN_DATA_FLOAT16
          ? std::array<std::uint16_t, 6>{0xc000, 0xbc00, 0xb800,
                                         0,      0x3800, 0x4000}
          : std::array<std::uint16_t, 6>{0xc000, 0xbf80, 0xbf00,
                                         0,      0x3f00, 0x4000};
  ...
  auto cudnn =
      pointwise_reference(FLAGDNN_POINTWISE_RELU_FWD,
                          {reference_tensor(input_uid, {elements}, data_type)},
                          reference_tensor(output_uid, {elements}, data_type));
  const auto expected = reference_cuda::run_contract_reference<std::uint16_t>(
      *cudnn, bindings, output_uid, elements, stream);
  execute_once(executable, bindings, stream);
  y.copy_to_host(output.data(), stream);
...
void test_add(const flagdnn::Executable& executable, CUstream stream) {
  constexpr int elements = 1024;
  ...
  const std::array<flagdnnBinding_t, 3> bindings = {
      flagdnnBinding_t{5, out.opaque()}, flagdnnBinding_t{3, x.opaque()},
      flagdnnBinding_t{4, y.opaque()}};
  auto cudnn = pointwise_reference(
      FLAGDNN_POINTWISE_ADD,
      {reference_tensor(3, {elements}), reference_tensor(4, {elements})},
      reference_tensor(5, {elements}));
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 5, elements, stream);
  execute_once(executable, bindings, stream);
...
void test_add_16bit(const flagdnn::Executable& executable, CUstream stream,
                    flagdnnDataType_t data_type, std::int64_t left_uid,
                    std::int64_t right_uid, std::int64_t output_uid,
                    const char* name) {
  if (data_type != FLAGDNN_DATA_FLOAT16 && data_type != FLAGDNN_DATA_BFLOAT16) {
  ...
  const std::array<std::uint16_t, 6> left_patterns =
      data_type == FLAGDNN_DATA_FLOAT16
          ? std::array<std::uint16_t, 6>{0x3c00, 0x4000, 0xbc00,
                                         0x3800, 0,      0xc000}
          : std::array<std::uint16_t, 6>{0x3f80, 0x4000, 0xbf80,
                                         0x3f00, 0,      0xc000};
  const std::array<std::uint16_t, 6> right_patterns =
      data_type == FLAGDNN_DATA_FLOAT16
          ? std::array<std::uint16_t, 6>{0x3c00, 0x3c00, 0x3800,
                                         0x3800, 0x3c00, 0x3c00}
          : std::array<std::uint16_t, 6>{0x3f80, 0x3f80, 0x3f00,
                                         0x3f00, 0x3f80, 0x3f80};
  ...
  auto cudnn =
      pointwise_reference(FLAGDNN_POINTWISE_ADD,
                          {reference_tensor(left_uid, {elements}, data_type),
                           reference_tensor(right_uid, {elements}, data_type)},
                          reference_tensor(output_uid, {elements}, data_type));
  const auto expected = reference_cuda::run_contract_reference<std::uint16_t>(
      *cudnn, bindings, output_uid, elements, stream);
  execute_once(executable, bindings, stream);
...
void test_add_strided_broadcast_alpha(const flagdnn::Executable& executable,
                                      CUstream stream) {
  ...
  std::vector<float> left(storage_element_count(left_dimensions, left_strides),
                          -1000.0F);
  ...
  for (std::size_t index = 0; index < logical_elements; ++index) {
    ...
    left[offset] = static_cast<float>(static_cast<int>(index) - 12) / 7.0F;
  }
  for (std::size_t index = 0; index < 4; ++index) {
    const std::size_t offset =
        logical_offset(index, right_dimensions, right_strides);
    right[offset] = static_cast<float>(static_cast<int>(index) - 2) / 5.0F;
  }
  for (std::size_t index = 0; index < logical_elements; ++index) {
  ...
  auto cudnn = pointwise_reference(
      FLAGDNN_POINTWISE_ADD,
      {reference_tensor(15, {2, 3, 4}, FLAGDNN_DATA_FLOAT32, {31, 9, 2}),
       reference_tensor(16, {1, 4}, FLAGDNN_DATA_FLOAT32, {13, 3})},
      reference_tensor(17, {2, 3, 4}, FLAGDNN_DATA_FLOAT32, {37, 11, 2}),
      alpha);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 17, output.size(), stream, output);
  execute_once(executable, bindings, stream);
  output_buffer.copy_to_host(output.data(), stream);
  check_cuda(cuStreamSynchronize(stream), "strided Add synchronize");

  require_close("add_strided_broadcast_alpha", output, expected, 1.0e-6F);
...
void test_reduction(const flagdnn::Executable& executable, CUstream stream) {
  ...
  auto cudnn = reduction_reference(reference_tensor(6, {rows, columns}),
                                   reference_tensor(7, {rows}), -1, false);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 7, rows, stream);
  execute_once(executable, bindings, stream);
  out.copy_to_host(output.data(), stream);
...
void test_reduction_middle_axis(const flagdnn::Executable& executable,
    ...
    for (int reduction_index = 0; reduction_index < reduction;
      ...
      for (int inner_index = 0; inner_index < inner; ++inner_index) {
        const int input_index =
            (outer_index * reduction + reduction_index) * inner + inner_index;
        const float value = static_cast<float>((input_index % 13) - 6) / 7.0F;
  ...
  auto cudnn =
      reduction_reference(reference_tensor(18, {outer, reduction, inner}),
                          reference_tensor(19, {outer, 1, inner}), 1, true);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 19, output.size(), stream);
  execute_once(executable, bindings, stream);
  out.copy_to_host(output.data(), stream);
  check_cuda(cuStreamSynchronize(stream), "middle-axis Reduction synchronize");
  require_close("reduction_sum_middle_axis_keepdim", output, expected, 2.0e-5F);
}

void test_reduction_mode(const flagdnn::Executable& executable, CUstream stream,
                         flagdnnReductionMode_t mode, const char* name) {
  constexpr int outer = 2;
  constexpr int reduction = 4;
  constexpr int inner = 3;
  std::vector<float> input(outer * reduction * inner);
  ...
  for (int outer_index = 0; outer_index < outer; ++outer_index) {
    for (int reduction_index = 0; reduction_index < reduction;
      ...
      for (int inner_index = 0; inner_index < inner; ++inner_index) {
        const int input_index =
            (outer_index * reduction + reduction_index) * inner + inner_index;
        const float value = static_cast<float>((input_index % 7) + 1) / 8.0F;
        input[input_index] = value;
      }
    }
  }
  ...
  const std::array<flagdnnBinding_t, 2> bindings = {
      flagdnnBinding_t{18, x.opaque()}, flagdnnBinding_t{19, out.opaque()}};
  auto cudnn = reduction_reference(
      reference_tensor(18, {outer, reduction, inner}),
      reference_tensor(19, {outer, 1, inner}), 1, true, mode);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 19, output.size(), stream);
  execute_once(executable, bindings, stream);
  out.copy_to_host(output.data(), stream);
...
void test_reduction_strided(const flagdnn::Executable& executable,
                            CUstream stream) {
  ...
  std::vector<float> output(
      ...
      padding_sentinel);
  std::vector<bool> output_positions(output.size(), false);
  for (std::size_t logical_index = 0; logical_index < logical_input_elements;
       ++logical_index) {
    const float value =
  ...
  for (std::size_t logical_index = 0; logical_index < logical_output_elements;
       ++logical_index) {
    output_positions[logical_offset(logical_index, output_dimensions,
                                    output_strides)] = true;
  }
  ...
  auto cudnn = reduction_reference(
      reference_tensor(26, {2, 4, 3}, FLAGDNN_DATA_FLOAT32, {12, 1, 4}),
      reference_tensor(27, {2, 1, 3}, FLAGDNN_DATA_FLOAT32, {7, 5, 2}), 1,
      true);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 27, output.size(), stream, output);
  execute_once(executable, bindings, stream);
  output_buffer.copy_to_host(output.data(), stream);
  check_cuda(cuStreamSynchronize(stream), "strided Reduction synchronize");

  require_close("reduction_sum_strided", output, expected, 2.0e-5F);
  for (std::size_t index = 0; index < output.size(); ++index) {
    if (!output_positions[index] && output[index] != padding_sentinel) {
...
void test_reduction_scalar(const flagdnn::Executable& executable,
                           CUstream stream) {
  std::vector<float> input = {-1.5F, 0.25F, 2.0F,   -3.25F,
                              4.5F,  1.0F,  -0.75F, 2.25F};
  ...
  auto cudnn = reduction_reference(reference_tensor(20, {8}),
                                   reference_tensor(21, {}), 0, false);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 21, 1, stream);
  execute_once(executable, bindings, stream);
  out.copy_to_host(output.data(), stream);
...
void test_reduction_16bit_middle_axis(const flagdnn::Executable& executable,
                                      CUstream stream,
                                      ...
                                      const char* name) {
  if (data_type != FLAGDNN_DATA_FLOAT16 && data_type != FLAGDNN_DATA_BFLOAT16) {
    throw std::invalid_argument("16-bit Reduction test received another dtype");
  }
  ...
  auto cudnn =
      reduction_reference(reference_tensor(input_uid, {1, 4, 2}, data_type),
                          reference_tensor(output_uid, {1, 1, 2}), 1, true);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, output_uid, output.size(), stream);
  execute_once(executable, bindings, stream);
  out.copy_to_host(output.data(), stream);
...
void test_conv(const flagdnn::Executable& executable, CUstream stream) {
  constexpr int n = 1;
  constexpr int c = 2;
  constexpr int h = 5;
  constexpr int w = 5;
  ...
  const std::array<flagdnnBinding_t, 3> bindings = {
      flagdnnBinding_t{8, x.opaque()}, flagdnnBinding_t{9, filter.opaque()},
      flagdnnBinding_t{10, out.opaque()}};
  reference::ConvolutionTestCase specification;
  specification.name = "runtime_conv2d";
  specification.x = reference_tensor(8, {n, c, h, w});
  specification.w = reference_tensor(9, {k, c, r, s});
  specification.y = reference_tensor(10, {n, k, oh, ow});
  specification.pre_padding = specification.post_padding = {1, 1};
  specification.stride = specification.dilation = {1, 1};
  auto cudnn = reference::build_convolution_reference(specification);
  const auto expected = reference_cuda::run_contract_reference<float>(
      *cudnn, bindings, 10, output.size(), stream);
  execute_once(executable, bindings, stream);
  out.copy_to_host(output.data(), stream);
...
void require_workspace_contract(
    const std::array<const flagdnn::Executable*, ExecutableCount>&
        executables) {
...
void test_relu_validation_contract(flagdnn::Handle const& handle) {
  const auto require_build_failure =
      [](auto&& build, flagdnnStatus_t expected_status, const char* message) {
        try {
...

  {
    ...
    require_build_failure([&] { return build_relu(handle, input, output); },
  ...
  {
    flagdnn::TensorDescriptor input(103, FLAGDNN_DATA_FLOAT32, dimensions,
                                    overlapping);
    flagdnn::TensorDescriptor output(104, FLAGDNN_DATA_FLOAT32, dimensions,
                                     contiguous);
    require_build_failure([&] { return build_relu(handle, input, output); },
  ...
  {
    flagdnn::TensorDescriptor input(105, FLAGDNN_DATA_FLOAT32, dimensions,
                                    contiguous);
...
void test_add_validation_contract(flagdnn::Handle const& handle) {
  ...
  }
  {
    ...
    require_build_failure(
        [&] { return build_add(handle, left, right, output); },
        FLAGDNN_STATUS_INVALID_VALUE, "mixed Add data types were not rejected");
  }
  {
    const std::array<std::int64_t, 2> dimensions = {2, 3};
    ...
    flagdnn::TensorDescriptor output(114, FLAGDNN_DATA_FLOAT32, dimensions,
                                     overlapping_strides);
    require_build_failure(
        [&] { return build_add(handle, left, right, output); },
        FLAGDNN_STATUS_NOT_SUPPORTED,
  ...
  for (const flagdnnDataType_t data_type :
    ...
    const std::array<std::int64_t, 2> dimensions = {2, 3};
    const std::array<std::int64_t, 2> overlapping_strides = {1, 1};
    flagdnn::TensorDescriptor left(uid++, data_type, dimensions,
                                   overlapping_strides);
...
void test_reduction_validation_contract(flagdnn::Handle const& handle) {
  ...
  }
  {
    ...
    flagdnn::TensorDescriptor input(121, FLAGDNN_DATA_FLOAT32, dimensions,
                                    overlapping_strides);
...
void test_conv_validation_contract(flagdnn::Handle const& handle) {
...

  {
    ...
    require_build_failure(
        [&] {
          return build_conv2d_fprop(handle, input, filter, padding, stride,
                                    dilation, 1, output);
        },
        FLAGDNN_STATUS_INVALID_VALUE,
  ...
  }
  {
    ...
    flagdnn::TensorDescriptor input(149, FLAGDNN_DATA_FLOAT32, dimensions,
                                    overlapping_strides);
...
int main(int argc, char** argv) {
  try {
    if (argc != 3) {
      std::cerr << "usage: native_nvidia_integration COMPILER_EXECUTABLE "
                   "COMPILER_ENTRY\n";
      return 2;
    }
    ...
    auto add_fp16_left = make_tensor(28, {1024}, FLAGDNN_DATA_FLOAT16);
    auto add_fp16_right = make_tensor(29, {1024}, FLAGDNN_DATA_FLOAT16);
    auto add_fp16_output = make_tensor(30, {1024}, FLAGDNN_DATA_FLOAT16);
    auto add_bf16_left = make_tensor(31, {1024}, FLAGDNN_DATA_BFLOAT16);
    auto add_bf16_right = make_tensor(32, {1024}, FLAGDNN_DATA_BFLOAT16);
    auto add_bf16_output = make_tensor(33, {1024}, FLAGDNN_DATA_BFLOAT16);
    ...
    flagdnn::TensorDescriptor strided_add_left(15, FLAGDNN_DATA_FLOAT32,
                                               strided_add_dimensions,
                                               strided_add_left_strides);
    flagdnn::TensorDescriptor strided_add_right(16, FLAGDNN_DATA_FLOAT32,
                                                strided_add_right_dimensions,
                                                strided_add_right_strides);
    flagdnn::TensorDescriptor strided_add_output(17, FLAGDNN_DATA_FLOAT32,
                                                 strided_add_dimensions,
    ...
    const std::array<std::int64_t, 3> strided_reduction_input_dimensions = {
        2, 4, 3};
    const std::array<std::int64_t, 3> strided_reduction_input_strides = {12, 1,
                                                                         4};
    const std::array<std::int64_t, 3> strided_reduction_output_dimensions = {
        2, 1, 3};
    const std::array<std::int64_t, 3> strided_reduction_output_strides = {7, 5,
                                                                          2};
    flagdnn::TensorDescriptor strided_reduction_input(
        26, FLAGDNN_DATA_FLOAT32, strided_reduction_input_dimensions,
        strided_reduction_input_strides);
    flagdnn::TensorDescriptor strided_reduction_output(
        27, FLAGDNN_DATA_FLOAT32, strided_reduction_output_dimensions,
    ...
    auto add_executable = build_add(handle, add_left, add_right, add_output);
    auto add_fp16_executable =
        build_add(handle, add_fp16_left, add_fp16_right, add_fp16_output);
    auto add_bf16_executable =
        build_add(handle, add_bf16_left, add_bf16_right, add_bf16_output);
    auto strided_add_executable = build_add_with_alpha(
        handle, strided_add_left, strided_add_right, strided_add_output, -0.75);
    auto reduction_executable = build_reduction_sum(handle, reduction_input, -1,
                                                    false, reduction_output);
    auto reduction_middle_executable = build_reduction_sum(
        handle, reduction_middle_input, 1, true, reduction_middle_output);
    auto reduction_avg_executable =
        build_reduction(handle, reduction_middle_input, FLAGDNN_REDUCTION_AVG,
                        1, true, reduction_middle_output);
    auto reduction_mul_executable =
        build_reduction(handle, reduction_middle_input, FLAGDNN_REDUCTION_MUL,
                        1, true, reduction_middle_output);
    auto strided_reduction_executable = build_reduction_sum(
        handle, strided_reduction_input, 1, true, strided_reduction_output);
    auto reduction_fp16_executable = build_reduction_sum(
        handle, reduction_fp16_input, 1, true, reduction_fp16_output);
    auto reduction_bf16_executable = build_reduction_sum(
        handle, reduction_bf16_input, 1, true, reduction_bf16_output);
    auto reduction_scalar_executable = build_reduction_sum(
        handle, reduction_scalar_input, 0, false, reduction_scalar_output);
    const std::array<std::int64_t, 2> padding = {1, 1};
    const std::array<std::int64_t, 2> stride = {1, 1};
    const std::array<std::int64_t, 2> dilation = {1, 1};
    auto conv_executable =
        build_conv2d_fprop(handle, conv_input, conv_filter, padding, stride,
                           dilation, 1, conv_output);
    require_workspace_contract(std::array<const flagdnn::Executable*, 16>{
        &relu_executable, &relu_fp16_executable, &relu_bf16_executable,
        &add_executable, &add_fp16_executable, &add_bf16_executable,
        &strided_add_executable, &reduction_executable,
        &reduction_middle_executable, &reduction_avg_executable,
        &reduction_mul_executable, &strided_reduction_executable,
        &reduction_fp16_executable, &reduction_bf16_executable,
        &reduction_scalar_executable, &conv_executable});
    ...
    auto cached_relu_bf16 =
        build_relu(handle, relu_bf16_input, relu_bf16_output);
    auto cached_add = build_add(handle, add_left, add_right, add_output);
    auto cached_add_fp16 =
        build_add(handle, add_fp16_left, add_fp16_right, add_fp16_output);
    auto cached_add_bf16 =
        build_add(handle, add_bf16_left, add_bf16_right, add_bf16_output);
    auto cached_strided_add = build_add_with_alpha(
        handle, strided_add_left, strided_add_right, strided_add_output, -0.75);
    auto cached_reduction = build_reduction_sum(handle, reduction_input, -1,
                                                false, reduction_output);
    auto cached_reduction_middle = build_reduction_sum(
        handle, reduction_middle_input, 1, true, reduction_middle_output);
    auto cached_reduction_avg =
        build_reduction(handle, reduction_middle_input, FLAGDNN_REDUCTION_AVG,
                        1, true, reduction_middle_output);
    auto cached_reduction_mul =
        build_reduction(handle, reduction_middle_input, FLAGDNN_REDUCTION_MUL,
                        1, true, reduction_middle_output);
    ...
    auto cached_conv =
        build_conv2d_fprop(handle, conv_input, conv_filter, padding, stride,
                           dilation, 1, conv_output);
    require_workspace_contract(std::array<const flagdnn::Executable*, 16>{
        &cached_relu, &cached_relu_fp16, &cached_relu_bf16, &cached_add,
        &cached_add_fp16, &cached_add_bf16, &cached_strided_add,
        &cached_reduction, &cached_reduction_middle, &cached_reduction_avg,
        &cached_reduction_mul, &cached_strided_reduction,
        &cached_reduction_fp16, &cached_reduction_bf16,
        &cached_reduction_scalar, &cached_conv});
    ...
    test_relu(relu_executable, caller_stream.get());
    test_relu_16bit(relu_fp16_executable, caller_stream.get(),
                    FLAGDNN_DATA_FLOAT16, 11, 12, "relu_fp16");
    test_relu_16bit(relu_bf16_executable, caller_stream.get(),
                    FLAGDNN_DATA_BFLOAT16, 13, 14, "relu_bfloat16");
    test_add(add_executable, caller_stream.get());
    test_add_16bit(add_fp16_executable, caller_stream.get(),
                   FLAGDNN_DATA_FLOAT16, 28, 29, 30, "add_fp16");
    test_add_16bit(add_bf16_executable, caller_stream.get(),
                   FLAGDNN_DATA_BFLOAT16, 31, 32, 33, "add_bfloat16");
    test_add_strided_broadcast_alpha(strided_add_executable,
                                     caller_stream.get());
    test_reduction(reduction_executable, caller_stream.get());
    test_reduction_middle_axis(reduction_middle_executable,
                               caller_stream.get());
    test_reduction_mode(reduction_avg_executable, caller_stream.get(),
                        FLAGDNN_REDUCTION_AVG,
                        "reduction_avg_middle_axis_keepdim");
    test_reduction_mode(reduction_mul_executable, caller_stream.get(),
                        FLAGDNN_REDUCTION_MUL,
                        "reduction_mul_middle_axis_keepdim");
    test_reduction_strided(strided_reduction_executable, caller_stream.get());
    test_reduction_16bit_middle_axis(reduction_fp16_executable,
                                     caller_stream.get(), FLAGDNN_DATA_FLOAT16,
                                     22, 23, "reduction_sum_fp16_middle_axis");
    test_reduction_16bit_middle_axis(
        reduction_bf16_executable, caller_stream.get(), FLAGDNN_DATA_BFLOAT16,
        24, 25, "reduction_sum_bfloat16_middle_axis");
    test_reduction_scalar(reduction_scalar_executable, caller_stream.get());
    test_conv(conv_executable, caller_stream.get());
