"""Distribution validation and launch selection for counter-based RNG."""
...
def _rng_kernel_configuration(parameters, tensors):
    ...
    seed = _require_integer(
        parameters, "seed", minimum=-(2**63), maximum=2**63 - 1
    )
    distribution = _require_integer(
        parameters, "distribution", minimum=1, maximum=3
    )
    ...
    if grid > 2**31 - 1:
        raise ValueError("RNG launch grid exceeds NVIDIA limits")
    return (
        "rng_kernel",
