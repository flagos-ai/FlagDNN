...
def build_compiler_identity(
    ...
    *,
    ...
    provider_root = provider_path.parent
    for package in ("dispatch", "codegen"):
        for module in sorted((provider_root / package).rglob("*.py")):
            relative = module.relative_to(provider_root)
            if "__pycache__" in relative.parts:
                continue
            if module.resolve() != Path(__file__).resolve():
                identity_inputs[
                    f"provider_module:nvidia:{relative.as_posix()}"
                ] = module
    ...
    for candidate in iter_kernel_candidates("nvidia"):
        # Common and platform registries may own modules with the same name.
        # Keep both dependencies, as a backend can select either in one graph.
        label = (
            f"kernel:{candidate.ownership}:{candidate.source_layout}:"
            f"{candidate.provider}:{candidate.source}"
        )
        identity_inputs[label] = resolve_kernel_source(
