...
def _resample_kernel_configuration(parameters, tensors):
    ...
    for tensor in tensors:
        if not _has_non_overlapping_strides(
            ...
            raise ValueError(
                "resample tensor strides or Hygon dimension "
                "limits are invalid"
            )
    if index and (
    ...
    if math.prod(window) > 2**31 - 1:
        raise ValueError("resample window volume exceeds Hygon kernel limits")
    if mode in (3, 4):
        if (
            window != [1] * spatial
            or stride != [1] * spatial
    ...
    if grid > 2**31 - 1:
        raise ValueError("resample launch grid exceeds Hygon limits")
    return (
        "resample_kernel",
        {
            ...
            "index_ptr": (
                "*i32" if index else TRITON_POINTER_TYPES[y["data_type"]]
            ),
        },
        constants,
        (grid, 1, 1),
