...
def _fp8_matmul_configuration(parameters, tensors):
    ...
    scale_type = (
        ("*u8" if mode == 2 else "*fp32")
        if mode
        else TRITON_POINTER_TYPES[c["data_type"]]
    )
    signature = dict(
    ...
    if grid > 2**31 - 1:
        raise ValueError("FP8 matmul launch grid exceeds Hygon limits")
