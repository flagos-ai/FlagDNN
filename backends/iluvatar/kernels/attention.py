...
@triton.jit
def _e4m3_to_float16(value):
    # Every finite E4M3 value is exactly representable in IEEE FP16. Construct
    # the FP16 bit pattern directly so CoreX never emits its packed B15 cast.
    bits = value.to(tl.int32)
    magnitude = bits & 0x7F
    exponent = (magnitude >> 3) & 0x0F
    mantissa = magnitude & 0x07
    subnormal_exponent = tl.where(
        mantissa >= 4, 8, tl.where(mantissa >= 2, 7, 6)
    )
    subnormal_base = tl.where(mantissa >= 4, 4, tl.where(mantissa >= 2, 2, 1))
    subnormal_shift = tl.where(
        mantissa >= 4, 8, tl.where(mantissa >= 2, 9, 10)
    )
    subnormal_bits = (subnormal_exponent << 10) | (
        (mantissa - subnormal_base) << subnormal_shift
    )
    subnormal_bits = tl.where(mantissa == 0, 0, subnormal_bits)
    normal_bits = ((exponent + 8) << 10) | (mantissa << 7)
    fp16_magnitude = tl.where(exponent == 0, subnormal_bits, normal_bits)
    fp16_magnitude = tl.where(magnitude == 0x7F, 0x7E00, fp16_magnitude)
    fp16_bits = fp16_magnitude | ((bits & 0x80) << 8)
    return fp16_bits.to(tl.int16).to(tl.float16, bitcast=True)


@triton.jit
def _e5m2_to_float16(value):
    # E5M2 and IEEE FP16 share the sign and exponent bias; shifting the two
    # mantissa bits into FP16 preserves zeros, subnormals, infinities and NaNs.
    bits = value.to(tl.int32) & 0xFF
    return (bits << 8).to(tl.int16).to(tl.float16, bitcast=True)


@triton.jit
def _float_to_e4m3(value):
    is_nan = value != value
    magnitude = tl.where(is_nan, 0.0, tl.minimum(tl.abs(value), 448.0))
    subnormal = magnitude < 0.015625
    safe_normal = tl.maximum(magnitude, 0.015625)
    exponent = tl.floor(tl.log2(safe_normal))
    normal_step = tl.exp2(exponent - 3.0)
    scaled = tl.where(subnormal, magnitude * 512.0, magnitude / normal_step)
    lower_float = tl.floor(scaled)
    lower = lower_float.to(tl.int32)
    fraction = scaled - lower_float
    increment = (fraction > 0.5) | ((fraction == 0.5) & ((lower & 1) != 0))
    rounded = lower + increment.to(tl.int32)
    normal_bits = (exponent.to(tl.int32) + 7) * 8 + rounded - 8
    magnitude_bits = tl.where(subnormal, rounded, normal_bits)
    magnitude_bits = tl.minimum(magnitude_bits, 0x7E)
    magnitude_bits = tl.where(is_nan, 0x7F, magnitude_bits)
    raw_float = value.to(tl.int32, bitcast=True)
    sign_bits = ((raw_float >> 31) & 1) * 0x80
    return (magnitude_bits | sign_bits).to(tl.int8)


@triton.jit
def _convert_fp8_to_fp16_4d_kernel(
    src_ptr,
    dst_ptr,
    n_elements,
    D1: tl.constexpr,
    ...
    SRC_STRIDE_1: tl.constexpr,
    SRC_STRIDE_2: tl.constexpr,
    SRC_STRIDE_3: tl.constexpr,
    FP8_E4M3: tl.constexpr,
    BLOCK: tl.constexpr,
):
    offsets = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offsets < n_elements
    index_3 = offsets % D3
    remaining = offsets // D3
    index_2 = remaining % D2
    remaining = remaining // D2
    index_1 = remaining % D1
    index_0 = remaining // D1
    source_offsets = (
        index_0 * SRC_STRIDE_0
        + index_1 * SRC_STRIDE_1
        + index_2 * SRC_STRIDE_2
        + index_3 * SRC_STRIDE_3
    )
    raw = tl.load(src_ptr + source_offsets, mask=mask, other=0)
    if FP8_E4M3:
        converted = _e4m3_to_float16(raw)
    else:
        converted = _e5m2_to_float16(raw)
    tl.store(dst_ptr + offsets, converted, mask=mask)
...
def _sdpa_fp8_fwd_inner(
    ...
    for start_n in tl.range(lo, hi, BLOCK_N):
        ...
        if FP8_E4M3:
            p_fp8 = _e4m3_to_float16(_float_to_e4m3(p * s_scale))
        else:
            p_fp8 = (p * s_scale).to(tl.float8e5).to(tl.float16)
...
def _sdpa_fp8_fwd_kernel(
    ...
    o_base = o_ptr + off_b * stride_ob + off_h * stride_oh
    if FP8_E4M3:
        o_output = _float_to_e4m3(o_val * o_scale)
    else:
        o_output = (o_val * o_scale).to(o_ptr.dtype.element_ty)
    tl.store(
        o_base + offs_m[:, None] * stride_om + offs_dv[None, :] * stride_od,
        o_output,
        mask=o_valid,
...
def _sdpa_fp8_bwd_dq_kernel(
    ...
    if FULL_BLOCKS:
        ...
        o = tl.load(o_base + o_offsets, mask=valid_md, other=0.0)
    ...
    for start_n in tl.range(0, loop_skv, BLOCK_N):
        ...
        if FP8_E4M3:
            ds_quant = _e4m3_to_float16(_float_to_e4m3(ds * scale_dp))
        else:
            ds_quant = (ds * scale_dp).to(tl.float8e5).to(tl.float16)
    ...
    if FP8_E4M3:
        dq_output = _float_to_e4m3(dq_val * scale_dq)
    else:
        dq_output = (dq_val * scale_dq).to(dq_ptr.dtype.element_ty)
    if FULL_BLOCKS:
        local_amax = tl.max(tl.abs(dq_val))
        tl.atomic_max(amax_dq_ptr, local_amax, sem="relaxed")
        tl.store(dq_out_ptrs, dq_output)
    else:
...
def _sdpa_fp8_bwd_dkdv_kernel(
    ...
    for group_idx in tl.static_range(0, Q_PER):
        ...
        for start_m in tl.range(loop_start_m, SQ, BLOCK_M):
            ...
            if FP8_E4M3:
                p_quant = _e4m3_to_float16(_float_to_e4m3(p * scale_s))
            else:
                p_quant = (p * scale_s).to(tl.float8e5).to(tl.float16)
    ...
    if FP8_E4M3:
        dk_output = _float_to_e4m3(dk_val * scale_dk)
        dv_output = _float_to_e4m3(dv_val * scale_dv)
    else:
        dk_output = (dk_val * scale_dk).to(dk_ptr.dtype.element_ty)
        dv_output = (dv_val * scale_dv).to(dv_ptr.dtype.element_ty)
    if FULL_BLOCKS:
        ...
        tl.store(dk_out_ptrs, dk_output)
        tl.store(dv_out_ptrs, dv_output)
    else:
