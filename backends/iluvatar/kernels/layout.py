...
@triton.jit
def dense_copy_kernel(
    input_ptr,
    output_ptr,
    n_elements,
    BLOCK_SIZE: tl.constexpr,
):
    """Copy a dense physical allocation without decoding logical indices."""

    offsets = tl.program_id(0).to(tl.int64) * BLOCK_SIZE + tl.arange(
        0, BLOCK_SIZE
    )
    active = offsets < n_elements
    value = tl.load(input_ptr + offsets, mask=active, other=0.0)
    tl.store(output_ptr + offsets, value, mask=active)
