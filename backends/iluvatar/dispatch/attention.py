...
"""Iluvatar dispatch / attention implementation."""

from __future__ import annotations

from .nn_common import ELEMENT_SIZES
from .nn_common import FLOAT_TYPES
from .nn_common import FP8_TYPES
from .nn_common import GridSpec
from .nn_common import INTERNAL_TUNING
from .nn_common import KernelStagePlan
from .nn_common import LOG2_E
from .nn_common import MAX_I32
from .nn_common import MAX_I64
from .nn_common import NodePlan
from .nn_common import PointerArgument
from .nn_common import SDPA_BACKWARD_TUNING
from .nn_common import SDPA_FP8_BACKWARD_TUNING
from .nn_common import SDPA_FP8_TUNING
from .nn_common import SDPA_TUNING
from .nn_common import ScalarArgument
from .nn_common import UNBOUNDED_DIAGONAL
...
from .nn_common import _require_flag
...
def _validate_qkv(
    ...
    *,
    ...
    if qd[1] % kd[1] or qd[1] % vd[1]:
        raise ValueError("SDPA query heads must be divisible by key/value heads")
    if qd[3] > 256 or vd[3] > 256:
        raise ValueError("SDPA head dimensions greater than 256 are unsupported")
    return {
        "data_type": data_type,
        "batch": qd[0],
        "heads": qd[1],
...
def _validate_bias(
    ...
    if (
        ...
        raise ValueError(f"{name} must broadcast over B/H and match Q/KV sequences")
...
def _validate_attention(node: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if operation == "sdpa_backward" and (shape["key_heads"] != shape["value_heads"]):
    ...
    if operation in ("sdpa_backward", "sdpa_fp8_backward"):
        _validate_attention_tensor(ports["do"], output_shape, data_type, "SDPA dO")
    ...
    stats = ports["stats"]
    if stats["data_type"] != "float32" or list(stats["dimensions"]) != stats_shape:
    ...
    for name in (
        ...
        if _require_integer(p, name, maximum=MAX_I64) != int(shape[name]):
            raise ValueError(f"parameters.{name} is inconsistent with Q/K/V metadata")
    min_diag = _require_integer(p, "min_diag", minimum=-MAX_I32, maximum=MAX_I32)
    max_diag = _require_integer(p, "max_diag", minimum=-MAX_I32, maximum=MAX_I32)
    if min_diag > max_diag:
        raise ValueError("SDPA diagonal interval is empty")
    banded = _require_flag(p, "banded")
    if banded != (min_diag != -UNBOUNDED_DIAGONAL or max_diag != UNBOUNDED_DIAGONAL):
    ...
    if operation in ("sdpa_backward", "sdpa_fp8_backward") and not generate_stats:
...
def _attention_forward_scalar_arguments(*, fp8: bool) -> tuple[ScalarArgument, ...]:
...
def _attention_forward_constants(
    ...
    if fp8:
        constants["FP8_E4M3"] = str(d["data_type"]) == "fp8_e4m3"
    else:
...
_ATTENTION_WORKSPACE_STRIDES = {
    "q": ("stride_qb", "stride_qh", "stride_qm", "stride_qd"),
    "k": ("stride_kb", "stride_kh", "stride_kn", "stride_kd"),
    "v": ("stride_vb", "stride_vh", "stride_vn", "stride_vd"),
    "bias": (
        "stride_bias_b",
        "stride_bias_h",
        "stride_bias_m",
    ...
    "o": ("stride_ob", "stride_oh", "stride_om", "stride_od"),
    "do": ("stride_dob", "stride_doh", "stride_dom", "stride_dod"),
}


def _apply_attention_workspace_strides(
    constants: dict[str, int | float | bool],
    port_name: str,
    workspace: WorkspaceTensor,
) -> None:
    for axis, (name, stride, dimension) in enumerate(
        zip(
            _ATTENTION_WORKSPACE_STRIDES[port_name],
            workspace.strides,
        ...
        constants[name] = (
            0 if port_name == "bias" and axis < 2 and dimension == 1 else stride
        )


def _fp8_input_workspaces(
    node: Mapping[str, Any], port_names: Sequence[str]
) -> tuple[dict[str, WorkspaceTensor], int]:
    ports = _require_object(node["port_tensors"], "node.port_tensors")
    result: dict[str, WorkspaceTensor] = {}
    cursor = 0
    for port_name in port_names:
        workspace = _workspace_tensor(
            f"{port_name}_fp16",
            "float16",
            ports[port_name]["dimensions"],
            offset=cursor,
        )
        result[port_name] = workspace
        cursor += _aligned_size(workspace.size)
    return result, cursor


def _raw_tensor_pointer(
    node: Mapping[str, Any], argument_name: str, port_name: str
) -> PointerArgument:
    indices = _require_object(node["port_indices"], "node.port_indices")
    return (argument_name, "*i8", "tensor", int(indices[port_name]))


def _fp8_conversion_stage(
    node: Mapping[str, Any],
    port_name: str,
    workspace: WorkspaceTensor,
    dependencies: tuple[str, ...],
    ...
    operation = str(node["operation"])
    tensor = node["port_tensors"][port_name]
    dimensions = tuple(int(value) for value in tensor["dimensions"])
    strides = tuple(int(value) for value in tensor["strides"])
    elements = _checked_product(dimensions, f"{port_name} FP8 elements")
    return _make_stage(
        operation=operation,
        stage_name=f"decode_{port_name}",
        function_name="_convert_fp8_to_fp16_4d_kernel",
        pointer_arguments=(
            _raw_tensor_pointer(node, "src_ptr", port_name),
            _workspace_pointer("dst_ptr", workspace.name, "float16"),
        ),
        constants={
            "n_elements": elements,
            "D1": dimensions[1],
            "D2": dimensions[2],
            "D3": dimensions[3],
            "SRC_STRIDE_0": strides[0],
            "SRC_STRIDE_1": strides[1],
            "SRC_STRIDE_2": strides[2],
            "SRC_STRIDE_3": strides[3],
            "FP8_E4M3": str(tensor["data_type"]) == "fp8_e4m3",
            "BLOCK": 256,
        },
        scalar_arguments=(("n_elements", "i32"),),
        tuning=INTERNAL_TUNING,
        tuning_key_value=elements,
        grid_spec=GridSpec("zero", (elements,)),
        dependencies=dependencies,
    )


def _attention_input_pointer(
    node: Mapping[str, Any],
    argument_name: str,
    port_name: str,
    workspaces: Mapping[str, WorkspaceTensor],
) -> PointerArgument:
    workspace = workspaces.get(port_name)
    if workspace is None:
        return _tensor_pointer(node, argument_name, port_name)
    return _workspace_pointer(argument_name, workspace.name, "float16")
...
def _fp8_forward_plan(node: Mapping[str, Any]) -> NodePlan:
    ...
    input_ports = ("q", "k", "v") + (("bias",) if bool(d["has_bias"]) else ())
    workspaces, workspace_size = _fp8_input_workspaces(node, input_ports)
    conversions = tuple(
        _fp8_conversion_stage(node, name, workspace, ("zero_amax",))
        for name, workspace in workspaces.items()
    )
    bias_port = "bias" if bool(d["has_bias"]) else "q"
    pointers = (
        _attention_input_pointer(node, "q_ptr", "q", workspaces),
        _attention_input_pointer(node, "k_ptr", "k", workspaces),
        _attention_input_pointer(node, "v_ptr", "v", workspaces),
        _attention_input_pointer(node, "bias_ptr", bias_port, workspaces),
        _tensor_pointer(node, "o_ptr", "o"),
        _tensor_pointer(node, "stats_ptr", "stats"),
        _tensor_pointer(node, "amax_s_ptr", "amax_s"),
        _tensor_pointer(node, "amax_o_ptr", "amax_o"),
    ...
    for name, workspace in workspaces.items():
        _apply_attention_workspace_strides(constants, name, workspace)
    forward = _make_stage(
        operation=operation,
        stage_name="forward",
        ...
        dependencies=("zero_amax",) + tuple(stage.stage_name for stage in conversions),
        num_stages=2,
    )
    plan = NodePlan(
        operation,
        (zero, *conversions, forward),
...
def _attention_backward_base(
    ...
    return {
        ...
        "BLOCK_M": 32,
        "BLOCK_N": 32,
        "BLOCK_D_FULL": _attention_full_dimension_block(int(d["head_dimension"])),
...
def _attention_backward_dq_stage(
    ...
    dbias_reduce = bool(
        has_dbias
        and (
            dbias["dimensions"][0] != d["batch"] or dbias["dimensions"][1] != d["heads"]
        )
    )
    ...
    constants.update(
        {
            ...
            "BLOCK_D_OUT": _attention_output_dimension_block(int(d["head_dimension"])),
            "BLOCK_DV": _attention_full_dimension_block(int(d["value_dimension"])),
            "HAS_DBIAS": has_dbias,
...
def _attention_backward_plan(node: Mapping[str, Any]) -> NodePlan:
    ...
    workspace_size = _ceil_div(raw_size, WORKSPACE_ALIGNMENT) * WORKSPACE_ALIGNMENT
    workspace = WorkspaceTensor(
        name="delta",
        data_type="float32",
    ...
    plan = NodePlan("sdpa_backward", tuple(stages), (workspace,), workspace_size)
    plan.validate_dependencies()
    return plan
...
def _fp8_backward_base(
    ...
    return {
        ...
        "FULL_BLOCKS": False,
        "CAUSAL_TOP_LEFT": bool(d["causal_top_left"]),
        "FP8_E4M3": str(d["data_type"]) == "fp8_e4m3",
    }


def _fp8_backward_dq_stage(
    node: Mapping[str, Any],
    workspaces: Mapping[str, WorkspaceTensor],
    dependencies: tuple[str, ...],
) -> KernelStagePlan:
    ...
    pointers = (
        ...
        _attention_input_pointer(node, "k_ptr", "k", workspaces),
        _attention_input_pointer(node, "v_ptr", "v", workspaces),
        _attention_input_pointer(node, "o_ptr", "o", workspaces),
        _attention_input_pointer(node, "do_ptr", "do", workspaces),
        _tensor_pointer(node, "stats_ptr", "stats"),
        _tensor_pointer(node, "dq_ptr", "dq"),
...
def _fp8_backward_dkdv_stage(
    node: Mapping[str, Any], workspaces: Mapping[str, WorkspaceTensor]
) -> KernelStagePlan:
    d = _require_object(node["derived"], "node.derived")
    dk = node["port_tensors"]["dk"]
    dv = node["port_tensors"]["dv"]
...
def _fp8_backward_plan(node: Mapping[str, Any]) -> NodePlan:
    ...
    workspaces, workspace_size = _fp8_input_workspaces(node, ("q", "k", "v", "o", "do"))
    conversions = tuple(
        _fp8_conversion_stage(node, name, workspace, ("zero_amax",))
        for name, workspace in workspaces.items()
    )
    dq_dependencies = ("zero_amax",) + tuple(stage.stage_name for stage in conversions)
    dq = _fp8_backward_dq_stage(node, workspaces, dq_dependencies)
    dk_dv = _fp8_backward_dkdv_stage(node, workspaces)
    plan = NodePlan(
        operation,
        (zero, *conversions, dq, dk_dv),
        tuple(workspaces.values()),
        workspace_size,
    )
    plan.validate_dependencies()
