...
def _hybrid_fmodf(left, right):
    ...
    requires_exact = algorithm_lane & (left_magnitude > right_magnitude * 1048576.0)
...
@triton.jit
def _apply_binary_elementwise_operation(
    left,
    right,
    OP_KIND: tl.constexpr,
    ALPHA: tl.constexpr,
...
@triton.jit
def binary_elementwise_contiguous_kernel(
    x_ptr,
    y_ptr,
    out_ptr,
    n_elements,
    ...
    result = _apply_binary_elementwise_operation(left, right, OP_KIND, ALPHA)
    tl.store(
        out_ptr + offsets,
        result.to(out_ptr.dtype.element_ty),
        mask=mask,
...
@triton.jit
def binary_elementwise_strided_kernel(
    x_ptr,
    y_ptr,
    out_ptr,
    n_elements,
