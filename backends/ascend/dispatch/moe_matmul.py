"""Ascend dispatch for moe matmul."""
...
def _moe_matmul_configuration(operation, parameters, tensors):
    ...
    if token["data_type"] in FP8_DATA_TYPES:
        raise ValueError("Ascend 910B does not support native FP8 MoE")
    ...
    ) or output["dimensions"] != ([experts, k, n] if backward else [1, routed, n]):
        raise ValueError("inconsistent MoE matrix or output shape")
    if mode != 1 and routed != tokens:
        raise ValueError("MoE routed and input counts must match outside gather mode")
    for tensor in tensors[3:-1]:
        if tensor["data_type"] != "int32" or tensor["dimensions"] != [
            ...
            raise ValueError("MoE routing indices must be INT32 [1,routed_tokens,1]")
    ...
    grid = (
        experts * ((k + 15) // 16) * ((n + 31) // 32)
        if backward
        else routed * ((n + 15) // 16)
    )
    if grid > 2**31 - 1:
        raise ValueError("MoE launch grid exceeds Ascend limits")
    return (
        ("moe_matmul_kernel" if backward else "moe_matmul_forward_kernel"),
        signature,
        constants,
        (grid, 1, 1),
        arguments,
