"""Ascend dispatch for normalization extended."""
...
def _extended_normalization_configuration(operation, parameters, tensors):
    ...
    if all(_is_row_major_contiguous(t) for t in tensors) and reduction <= 2048:
        signature = {
    ...
    if grid > 2**31 - 1:
        raise ValueError("normalization launch grid exceeds Ascend limits")
    return (
        (
