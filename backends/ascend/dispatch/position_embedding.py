"""Ascend dispatch for position embedding."""
...
def _rope_kernel_configuration(operation, parameters, tensors):
    ...
    width = _require_integer(parameters, "rope_dim", minimum=2, maximum=dimension)
    if (
        width % 2
        ...
        raise ValueError(
            "RoPE frequencies must be FP32 [S,1,1,rope_dim] " "with even rotation width"
        )
    ...
    if grid > 2**31 - 1:
        raise ValueError("RoPE launch grid exceeds Ascend limits")
    return (
        "rope_kernel",
        {
