...
int run_extended_normalization_functional_test(
    int argc, char** argv,
    std::span<const ExtendedNormalizationTestCase> cases, bool benchmark) {
  return mthreads::run_paired_cases(
      argc, argv, cases, "FLAGDNN_NORMALIZATION_EXTENDED_CASE",
      cases.front().operation, build_flagdnn_extended_normalization,
      [](const ExtendedNormalizationTestCase& test_case) {
        ...
        for (const auto& tensor : test_case.inputs) {
          ...
          for (std::size_t index = 0; index < values.size(); ++index) {
            values[index] =
                static_cast<float>(
                    static_cast<int>((index * 17 + tensor.uid * 7) % 61) -
                    30) /
                19.0F;
