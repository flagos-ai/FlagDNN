"""Grouped GEMM layout checks, routing ABI and MThreads launch geometry."""
...
def _moe_matmul_configuration(operation, parameters, tensors, architecture):
    ...
    if token["data_type"] in FP8_DATA_TYPES and architecture < 31:
        raise ValueError("MThreads FP8 MoE requires MUSA cc31 or newer")
    ...
    if grid > 2**31 - 1:
        raise ValueError("MoE launch grid exceeds MThreads limits")
