...
def torch_any(x, dim, keepdim):
    if dim is None:
        return torch.any(x)
    return torch.any(x, dim=dim, keepdim=keepdim)


def gems_any_wrapper(x, dim, keepdim):
    if dim is None:
        return flag_dnn.ops.any(x)
    return flag_dnn.ops.any(x, dim=dim, keepdim=keepdim)
