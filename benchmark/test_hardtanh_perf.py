...
def torch_hardtanh(x, min_val, max_val):
    return F.hardtanh(x, min_val=min_val, max_val=max_val)


def gems_hardtanh_wrapper(x, min_val, max_val):
    return flag_dnn.ops.hardtanh(x, min_val, max_val)


class HardtanhBenchmark(Benchmark):
    IO_FACTOR = 2
...
def test_perf_hardtanh(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")

    bench = HardtanhBenchmark(
        op_name="hardtanh",
        torch_op=torch_hardtanh,
        gems_op=gems_hardtanh_wrapper,
        dtypes=[dtype],
    )
    bench.run()
