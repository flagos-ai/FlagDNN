...
def gems_minimum_wrapper(x, y):
    return flag_dnn.ops.minimum(x, y)
...
def test_perf_minimum(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")

    bench = MinimumBenchmark(
        op_name="minimum",
        torch_op=torch_minimum,
        gems_op=gems_minimum_wrapper,
        dtypes=[dtype],
    )
    bench.run()
