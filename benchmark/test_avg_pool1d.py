...
def avg_pool1d_input_fn(case, dtype, device, max_peak_bytes):
...
def avg_pool1d_output_numel(args):
...
def test_avg_pool1d():
    bench = base.PoolingBenchmark(
        op_name="avg_pool1d",
        torch_op=F.avg_pool1d,
        dtypes=consts.FLOAT_DTYPES,
        input_fn=avg_pool1d_input_fn,
        output_numel_fn=avg_pool1d_output_numel,
        cases=AVG_POOL1D_CASES,
    )
    bench.run()
