...
def torch_bitwise_not(x):
    return torch.bitwise_not(x)


def gems_bitwise_not_wrapper(x):
    return flag_dnn.ops.bitwise_not(x)


class Bitwise_notBenchmark(Benchmark):
...
def test_perf_bitwise_not(dtype):
    bench = Bitwise_notBenchmark(
        op_name="bitwise_not",
        torch_op=torch_bitwise_not,
        gems_op=gems_bitwise_not_wrapper,
        dtypes=[dtype],
    )
    bench.run()
