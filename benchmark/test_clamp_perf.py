...
class ClampBenchmark(Benchmark):
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        ...
        for shape_x, shape_min, shape_max in self.shapes:
            ...
            bytes_min = np.prod(shape_min) * element_size if shape_min is not None else 0
            bytes_max = np.prod(shape_max) * element_size if shape_max is not None else 0
            ...
            if shape_min is None:
                ...
                min_val = torch.randn(shape_min, dtype=cur_dtype, device=self.device) - 2.0
            ...
            if shape_max is None:
                ...
                # 偏移到正数区间
                max_val = torch.randn(shape_max, dtype=cur_dtype, device=self.device) + 2.0
