...
class RmsNormBenchmark(Benchmark):
    ...
    def set_more_shapes(self):
        configs = [
            ...
            ((32, 1, 8192), (8192,)),              # 70B/72B 级别大模型 (如 LLaMA-3-70B, Qwen2-72B) 的解码
            ((1, 1, 4096), (4096,)),               # 极低延迟的单用户流式响应

            # 2. Flattened Tokens (连续批处理 Continuous Batching / PageAttention 场景)
            # 特点：现代推理框架为了消除 Padding，通常会将 (Batch, SeqLen, Hidden) 展平为 (Total_Tokens, Hidden)。
            ...
            # 特点：很多模型的 Hidden Size 不是完美的 1024, 2048, 4096，这非常考验 Kernel 内的向量化读取 (Vectorized Load) 和边界处理。
            ((16, 2048, 1536), (1536,)),           # Qwen2-1.5B (Hidden = 1536)
            ((16, 2048, 3072), (3072,)),           # Phi-3-mini (Hidden = 3072)
            ((8, 4096, 5120), (5120,)),            # LLaMA-13B
            ((8, 4096, 3584), (3584,)),            # LLaMA-3-8B 的 FFN 层有时会涉及非标准维度的投影归一化
