...
class LayerNormBenchmark(Benchmark):
    ...
    def set_more_shapes(self):
        ...
        configs = [
            ...
            ((4, 8192, 2048), (2048,)),            # 小规模模型长文本预填充 (如 Qwen 1.5B, 8K 上下文)
            ((2, 32768, 4096), (4096,)),           # 长上下文场景 (如 Llama-3-8B 处理 32K Token)
            ((1, 128000, 4096), (4096,)),          # 极长上下文场景 (128K Token，此时 Batch 往往受限于显存只能为 1)
            ((8, 4096, 8192), (8192,)),            # 百亿级参数模型 (70B+) 标准预训练尺寸
            
            # 6. 现代视觉与生成式模型 (Swin, ConvNeXt, DiT)
            ((16, 256, 1152), (1152,)),            # DiT-XL (Diffusion Transformer, 如 Sora 的基础架构), 16x16=256 个 Patch
            ((2048, 49, 96), (96,)),               # Swin Transformer 窗口注意力前置的 LN: (B*NumWindows, WindowArea, C), 7x7窗口
            ((32, 56, 56, 96), (96,)),             # ConvNeXt-Tiny: 典型的 (B, H, W, C) 格式，仅在通道维度 C 上做 LayerNorm
