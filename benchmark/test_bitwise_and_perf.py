...
def torch_bitwise_and(x, y):
    return torch.bitwise_and(x, y)


def gems_bitwise_and_wrapper(x, y):
    return flag_dnn.ops.bitwise_and(x, y)


class Bitwise_andBenchmark(Benchmark):
...
def test_perf_bitwise_and(dtype):
    bench = Bitwise_andBenchmark(
        op_name="bitwise_and",
        torch_op=torch_bitwise_and,
        gems_op=gems_bitwise_and_wrapper,
        dtypes=[dtype],
    )
    bench.run()
