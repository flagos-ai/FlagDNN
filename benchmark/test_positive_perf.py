...
def torch_positive(x):
    return torch.positive(x)


def gems_positive_wrapper(x):
    return flag_dnn.ops.positive(x)


class PositiveBenchmark(Benchmark):
...
def test_perf_positive(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")

    bench = PositiveBenchmark(
        op_name="positive",
        torch_op=torch_positive,
        gems_op=gems_positive_wrapper,
        dtypes=[dtype],
    )
    bench.run()
