...
def gems_maximum_wrapper(x, y):
    return flag_dnn.ops.maximum(x, y)
...
def test_perf_maximum(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")

    bench = MaximumBenchmark(
        op_name="maximum",
        torch_op=torch_maximum,
        gems_op=gems_maximum_wrapper,
        dtypes=[dtype],
    )
    bench.run()
