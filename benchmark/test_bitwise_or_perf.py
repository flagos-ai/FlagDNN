...
def torch_bitwise_or(x, y):
    return torch.bitwise_or(x, y)


def gems_bitwise_or_wrapper(x, y):
    return flag_dnn.ops.bitwise_or(x, y)


class Bitwise_orBenchmark(Benchmark):
...
def test_perf_bitwise_or(dtype):
    bench = Bitwise_orBenchmark(
        op_name="bitwise_or",
        torch_op=torch_bitwise_or,
        gems_op=gems_bitwise_or_wrapper,
        dtypes=[dtype],
    )
    bench.run()
