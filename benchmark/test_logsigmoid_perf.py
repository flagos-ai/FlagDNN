...
def gems_logsigmoid_wrapper(x):
    return flag_dnn.ops.logsigmoid(x)


class LogSigmoidBenchmark(Benchmark):
    IO_FACTOR = 2
...
def test_perf_logsigmoid(dtype):
    if dtype == torch.float64 and not flag_dnn.runtime.device.support_fp64:
        pytest.skip("Device does not support float64")

    bench = LogSigmoidBenchmark(
        op_name="logsigmoid",
        torch_op=torch_logsigmoid,
        gems_op=gems_logsigmoid_wrapper,
        dtypes=[dtype],
    )
    bench.run()
