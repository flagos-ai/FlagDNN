...
| graph op | `src/flag_dnn/graph/registry/core.py`、`registry/schemas/`、`registry/schemas/__init__.py`、`registry/ops.py`、`registry/__init__.py`、`wrappers.py` | capture metadata、shape/dtype 推导、executor fallback/registration；`registry/schemas/__init__.py` 聚合 family schema，`registry/ops.py` 按 `_register_*_ops` 注册，`registry/__init__.py` 是兼容导出入口 |
...
| cuDNN 功能对标 | `tests/test_<op>.py` |
| cuDNN graph 性能对标 | `benchmark/test_<op>_perf.py` |
| graph 子图对标 | `tests/test_graph_<pattern>.py` |
...
- `缺失`：没有明确对应实现。
- `Graph缺`：eager 有，但 `src/flag_dnn/graph/registry/core.py` / `registry/schemas/` / `registry/ops.py` / `wrappers.py` 未覆盖或只覆盖子集。
...
| `min` | `input0, input1, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style `min` graph alias；复用 `minimum` Triton 实现，`tests/benchmark` 已覆盖 | `ops/min.py` + `ops/minimum.py`；graph registry |
| `mod` | `input0, input1, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/mod.py` |
| `cmp_eq` | `input, comparison, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style alias，复用 `eq`；Graph 有，`tests/benchmark` 已覆盖 | 已有 |
| `cmp_neq` | `input, comparison, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style alias，复用 `ne`；Graph 有，`tests/benchmark` 已覆盖 | 已有 |
| `cmp_gt` | `input, comparison, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style alias，复用 `gt`；Graph 有，`tests/benchmark` 已覆盖 | 已有 |
| `cmp_ge` | `input, comparison, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style alias，复用 `ge`；Graph 有，`tests/benchmark` 已覆盖 | 已有 |
| `cmp_lt` | `input, comparison, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style alias，复用 `lt`；Graph 有，`tests/benchmark` 已覆盖 | 已有 |
| `cmp_le` | `input, comparison, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style alias，复用 `le`；Graph 有，`tests/benchmark` 已覆盖 | 已有 |
...
| `binary_select` | `input0, input1, mask, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/binary_select.py`；`where` 仅作为兼容 alias |
| `add_square` | `a, b, compute_data_type=NOT_SET, name=''` | 已有 cudnn-style graph 封装；复用 `square` + `add`，`tests/benchmark` 已覆盖 | `ops/add_square.py`，语义 `a + b*b` |
...
| `rsqrt` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/rsqrt.py` |
| `reciprocal` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/reciprocal.py` |
| `ceil` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/ceil.py` |
| `floor` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/floor.py` |
| `exp` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/exp.py` |
| `log` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/log.py` |
| `erf` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/erf.py` |
| `sin` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/sin.py` |
| `cos` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/cos.py` |
| `tan` | `input, compute_data_type=NOT_SET, name=''` | 已有 Triton + Graph；`tests/benchmark` 已覆盖 | `ops/tan.py` |
...
| `causal_conv1d` high-level | `x, weight, bias=None, activation='identity'` | 已有 eager + Graph + tests + benchmark；支持 depthwise causal padding、bias、`identity`/`silu` | 已有 |
...
| `sdpa_fp8_backward` | `q, k, v, o, dO, stats, descale_q, descale_k, descale_v, descale_o, descale_dO, descale_s, descale_dP, scale_s, scale_dQ, scale_dK, scale_dV, scale_dP, ...` | 已实现，性能待继续优化 | `ops/sdpa_fp8_backward.py` |
...
src/flag_dnn/graph/registry/core.py
src/flag_dnn/graph/registry/schemas/<family>.py
src/flag_dnn/graph/registry/schemas/__init__.py   # family schema 聚合入口
src/flag_dnn/graph/registry/ops.py
src/flag_dnn/graph/registry/__init__.py          # 兼容导出入口
src/flag_dnn/graph/wrappers.py
tests/test_<op>.py
tests/test_<op>.py
benchmark/test_<op>_perf.py
```
...
1. `src/flag_dnn/graph/registry/core.py`
   - 在 `GRAPH_OP_METADATA` 中登记需要 capture 的 graph-aware op；如果 eager API 返回 dict，用 `output_keys` 登记输出 key。
   - `wrappers.py` 会从 metadata 自动安装 wrapper，通常不需要新增 op 时手动维护 wrapper 列表。

2. `src/flag_dnn/graph/registry/schemas/<family>.py` / `src/flag_dnn/graph/registry/ops.py`
   - family schema 文件放 `_normalize_<op>` 和 `_<op>_shape`，shape/dtype 推导必须准确，并通过 `__all__` 交给 `registry/schemas/__init__.py` 聚合。
   - `registry/ops.py` 放 `_run_<op>`，并在对应 `_register_*_ops` helper 中做 `OpSchema(name, normalize_fn, shape_fn, run_fn, fusible=...)` 注册。
   - multi-output op 已由 `Graph.add_op` 和 `OpSchema.num_outputs` 支持；固定输出用整数，可变输出用 tuple 或 `None`；新增时 `shape_fn` 要返回合法数量的 `TensorSpec`，`run_fn` 要返回等长 tuple/list。
   - `src/flag_dnn/graph/registry/__init__.py` 只保留兼容导出入口。
...
1. 清理 graph alias hygiene 和边界 coverage，继续同步 `tests` 与 `benchmark`。
...
| `src/flag_dnn/graph/registry/schemas/matmul_attention.py`、`registry/ops.py` | graph op 接入。`matmul_attention.py` 中的 `_normalize_sdpa` 负责把 cuDNN/Python 参数标准化成 graph attrs，并把 `q/k/v/bias` 转成 graph value；`_sdpa_shape` 负责输出和可选 stats 的 shape/dtype 推导；`registry/ops.py` 中的 `_run_sdpa` 负责 executor fallback 调用 eager op；`_register_matmul_attention_ops` 中的 `OpSchema(name="sdpa", num_outputs=(1, 2), fusible=True)` 负责注册。新增参数时这里必须和 eager API 同步。 |
| `src/flag_dnn/graph/registry/core.py`、`wrappers.py` | 在 `GRAPH_OP_METADATA` 中登记 `"sdpa"`；`wrappers.py` 会按 metadata 自动安装 wrapper，保证 `@flag_dnn.graph` capture 时调用的是 graph op，而不是直接执行 eager op。 |
| `src/flag_dnn/graph/prepared/sdpa_forward.py` | graph 编译期 runner。通过 prepared 注册机制路由到 SDPA forward preparer；preparer 在 shape/stride/attrs 静态可知时绑定 scalar 参数、选择 fast path、缓存已编译 kernel 和 grid，运行时只分配输出并 launch kernel。当前 forward prepared path 覆盖 decode、GQA causal、dense exact 和 generic；输入 stride 或 bias stride 不匹配时 fallback 到 registry 的默认 run_fn。新增 forward fast path 时这里要和 `ops/sdpa.py` 的 eager dispatch 保持一致。 |
...
| `src/flag_dnn/graph/registry/schemas/matmul_attention.py`、`registry/ops.py` | `matmul_attention.py` 中的 `_normalize_sdpa_backward` 对齐 eager/cudnn 参数，处理 `bias/dBias` 可选输入并拒绝暂不支持的 seq_len/dropout/score callback/sink token 等；`_sdpa_backward_shape` 推导 `dQ/dK/dV`；`registry/ops.py` 中的 `_run_sdpa_backward` 调 eager backward；`_register_matmul_attention_ops` 中的 `OpSchema(name="sdpa_backward", num_outputs=3)` 负责注册。 |
| `src/flag_dnn/graph/registry/core.py`、`wrappers.py` | 在 `GRAPH_OP_METADATA` 中登记 `"sdpa_backward"`；`wrappers.py` 会按 metadata 自动安装 wrapper。 |
| `src/flag_dnn/graph/prepared/sdpa_backward.py` | SDPA backward graph 编译期 fast path：静态绑定 shape/stride/attrs，选择 fused atomic、causal、GQA、triangular、exact/delta 等路径，缓存 kernel/grid，并在运行时 fallback 到默认 graph run_fn。新增 backward kernel 时要同步 eager dispatch 和 prepared dispatch。 |
...
| 文件 | 需要实现/维护的内容 |
|---|---|
| `tests/test_graph_sdpa.py` | forward cuDNN graph 对标测试。覆盖默认 scale、`generate_stats`、inference 单输出、GQA decode、GQA causal D128、causal/top-left、bottom-right causal、sliding window、bias、fp32 bias torch fallback、显式 `attn_scale`、causal shorthand 等。新增 forward 语义时补这里。 |
| `tests/test_graph_sdpa_backward.py` | backward cuDNN graph 对标测试。覆盖 dense backward、causal backward、bias + dBias 等，并断言 graph capture 后节点类型是 `sdpa_backward`。新增 backward 语义时补这里。 |
| `tests/consts.py` | graph 正确性用例集合。forward 主要维护 `SDPA_CASES`、`SDPA_MASKED_CASES`；新增 shape、GQA/MQA、D128、seq_q/seq_kv 边界时同步这里。 |
| `benchmark/test_sdpa_perf.py` | forward graph 性能对标。构造 cuDNN graph runner 和 FlagDNN compiled graph runner，断言节点是 `sdpa`，并按 benchmark consts 跑 dtype/shape。新增 forward fast path 后应加性能覆盖。 |
| `benchmark/test_sdpa_backward_perf.py` | backward graph 性能对标。用 `flag_dnn.sdpa(..., generate_stats=True)` 生成 backward 输入，比较 cuDNN graph 和 FlagDNN compiled graph，断言节点是 `sdpa_backward`。新增 backward fast path 后应加性能覆盖。 |
| `benchmark/consts.py` | benchmark shape 集合和编译选项。新增性能场景时同步 `SDPA_SHAPES`、`SDPA_BACKWARD_SHAPES` 等。 |
...
3. 在 `graph/registry/core.py` 登记 capture metadata，在 `graph/registry/schemas/<family>.py` 补 normalize/shape，在 `graph/registry/ops.py` 补 run_fn，并在对应 `_register_*_ops` helper 中注册 `OpSchema`，特别注意 multi-output/variable-output 的 `num_outputs`、可选输入顺序、attrs 名称和 fallback 行为。
4. 对热点场景在 `graph/prepared/<family>.py` 加 prepared fast path，把 shape/stride/attrs、kernel constexpr、grid 和缓存逻辑静态化；不满足约束时必须 fallback 到默认 run_fn。
5. 给新 kernel 在 `runtime/backend/_nvidia/tune_configs.yaml` 加 tune key，并让 eager dispatch 和 prepared dispatch 使用同一组配置名。
6. 最后补 `tests` 正确性、`benchmark` 性能和 consts shape 集合；每个新语义至少要覆盖 eager graph capture、cuDNN 对标或明确的 torch fallback，以及 compiled graph 节点断言。
