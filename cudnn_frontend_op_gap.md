...
5. FlagDNN graph registry 已覆盖基础 utility、二元/比较/部分 unary/activation、`conv_fprop`、`matmul`、`reduction` 和 norm forward；剩余差距主要集中在 missing pointwise、`binary_select`、backward、attention、低精度和 MoE wrapper。

## 3. 优先级建议

| 优先级 | 目标 | 原因 |
|---|---|---|
| P0 | remaining pointwise + `binary_select` + graph alias hygiene | utility 主体已完成，剩余缺口成本低，能继续扩大 graph coverage |
...
这些 unary op 可共用现有 `src/flag_dnn/ops/unary.py` 或 `utils/pointwise_dynamic.py` 模板，优先不要每个文件重复写大段 Triton kernel。补 `reciprocal/ceil/floor/erf/sin/cos/tan` 时建议扩展现有模板。
...
以下只列仍缺失或需要新增的文件。`identity/reshape/transpose/slice/concatenate/gen_index/exp/log/rsqrt/sigmoid/sigmoid_backward/logical_*`、`matmul/conv_fprop/reduction`、norm forward 和 `rmsnorm_rht_amax.py` 等当前已存在，不再作为新增项列出。
...
src/flag_dnn/ops/where.py        # cuDNN binary_select
```
...
1. 补剩余 P0 pointwise、`mod` 和 `binary_select/where`，同步 `tests_graph` 与 `benchmark_graph`。
